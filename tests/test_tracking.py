...
class TestTrackingRules(unittest.TestCase):
    def setUp(self):
        self.test_gaussians = os.listdir("test_gaussians/")
    ...
    def test_coordinates(self):
        coordinates, _ = get_coordinates(cv.imread("../tests/test_gaussians/" + self.test_gaussians[1], 0) / 255)
        self.assertEqual(len(coordinates), 9)

    def test_cell_initialization(self):
        cell_list = []
        coordinates, _ = get_coordinates(cv.imread("../tests/test_gaussians/" + self.test_gaussians[2], 0) / 255)
    ...
    def test_proximity_kill(self):
        ...
        for n, frame in enumerate(self.test_gaussians[23:33]):
            coordinates, _ = get_coordinates(cv.imread("../tests/test_gaussians/" + frame, 0) / 255)
    ...
    def test_cell_completion(self):
        for n, frame in enumerate(self.test_gaussians[:]):
            coordinates, _ = get_coordinates(cv.imread("../tests/test_gaussians/" + frame, 0) / 255)
            cv.imwrite("tt_{}.png".format(str(n)), _)
